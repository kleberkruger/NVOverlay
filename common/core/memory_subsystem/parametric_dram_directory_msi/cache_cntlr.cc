...
#include "cache_cntlr_nvover.h"  // Added by Kleber Kruger
...
MYLOG("writethrough start");
            // Modified by Kleber Kruger (m_next_cache_cntlr->writeCacheBlock() -> writeCacheBlockToNextLevel())
            writeCacheBlockToNextLevel(ca_address, offset, data_buf, data_length, ShmemPerfModel::_USER_THREAD);
...
MYLOG("evicting @%lx", evict_address);
      ...
      else if (m_next_cache_cntlr)
      {
         if (m_cache_writethrough) {
            ...
            {  
               // Modified by Kleber Kruger (m_next_cache_cntlr->writeCacheBlock() -> writeCacheBlockToNextLevel())
               writeCacheBlockToNextLevel(evict_address, 0, evict_buf, getCacheBlockSize(), thread_num);
            }
         }
   ...
   else
   {
      ...
      if (cache_block_info->getCState() == CacheState::MODIFIED) {
         ...
         if (m_cache_writethrough) {
            ...
            // Modified by Kleber Kruger (m_next_cache_cntlr->writeCacheBlock() -> writeCacheBlockToNextLevel())
            writeCacheBlockToNextLevel(address, 0, data_buf, getCacheBlockSize(), thread_num);
            is_writeback = true;
            sibling_hit = true;
...
assert(data_length==getCacheBlockSize());
   ...
   if (m_cache_writethrough) {
      // Fixed by Kleber Kruger (added ifdef to lock on PRIVATE_L2_OPTIMIZATION mode)
      #ifdef PRIVATE_L2_OPTIMIZATION
      acquireStackLock(address, true);
      #endif
      // Modified by Kleber Kruger (m_next_cache_cntlr->writeCacheBlock() -> writeCacheBlockToNextLevel())
      writeCacheBlockToNextLevel(address, offset, data_buf, data_length, thread_num);
      #ifdef PRIVATE_L2_OPTIMIZATION
      releaseStackLock(address, true);
      #endif
      // End of fix
...
void
CacheCntlr::writeCacheBlockToNextLevel(IntPtr address, UInt32 offset, Byte* data_buf, UInt32 data_length, ShmemPerfModel::Thread_t thread_num)
{
...
/**
 * @brief Create a CacheCntlr to specified project
...
CacheCntlr *CacheCntlr::create(MemComponent::component_t mem_component,
                               ...
                               bool is_last_level_cache)
{
   if (Sim()->getProjectType() == ProjectType::NVOVERLAY)
   {
      return new CacheCntlrNVOver(mem_component, name, core_id, memory_manager, tag_directory_home_lookup, user_thread_sem, 
   ...
   else if (Sim()->getCfg()->getBoolDefault("perf_model/" + cache_params.configName + "/writebuffer/enabled", false))
   {
