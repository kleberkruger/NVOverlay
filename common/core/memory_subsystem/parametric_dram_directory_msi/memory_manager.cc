...
MemoryManager::MemoryManager(Core* core,
   ...
   m_nvm_cntlr_present(false), // Added by Kleber Kruger
   m_enabled(false)
{
   ...
   if (Sim()->getProjectType() == ProjectType::NVOVERLAY) {
      m_multisnap_map = PrL1PrL2DramDirectoryMSI::MultiSnapshotNVMMapping::getInstance();
   }
}
   ...
   switch (receiver_mem_component)
   {
      ...
      case MemComponent::NVM:
         ...
         switch(sender_mem_component)
         {
            ...
            {
               if (Sim()->getProjectType() == ProjectType::NVOVERLAY)
                  static_cast<PrL1PrL2DramDirectoryMSI::OverlayMemCntlr*>(m_dram_cntlr)->handleMsgFromLLC(sender, shmem_msg);
               break;
            }
            default:
...
MemoryManager::enableModels()
{
   ...
   if (m_dram_cntlr_present)
      m_dram_cntlr->enableDramPerfModel(true); // Modified by Kleber Kruger (before: m_dram_cntlr->getDramPerfModel()->enable())
}
...
MemoryManager::disableModels()
{
   ...
   if (m_dram_cntlr_present)
      m_dram_cntlr->enableDramPerfModel(false); // Modified by Kleber Kruger (before: m_dram_cntlr->getDramPerfModel()->disable())
}
